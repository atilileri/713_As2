...
main(int argc, char ** argv)
{
	...
	std::cout << "####### ORIGINAL NPP VERSION #######" << std::endl;
	...
	std::cout << "Cons: " << (static_cast<unsigned int>(nConstant) & 0xFF) << " SF : " << (static_cast<unsigned int>(nScaleFactor) & 0xFF) << std::endl;
	...
	WritePGM("..\\output\\lena_after_ORG.pgm", pDst_Host, nWidth, nHeight, nMaxGray);
